...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", disable_tqdm=False):
    ...
    results = {
        "loss": eval_loss,
        "precision": 0 if mode =='test' else precision_score(out_label_list, preds_list),
        "recall": 0 if mode =='test' else recall_score(out_label_list, preds_list),
        "f1": 0 if mode =='test' else f1_score(out_label_list, preds_list),
        "flat_f1": 0 if mode =='test' else metrics.flat_f1_score(out_label_list, preds_list, average='micro', labels=[label for label in labels if label != 'O'])
    }
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    if args.use_positions:
        all_positions = torch.tensor([f.token_positions for f in features], dtype=torch.float)
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_positions)
    else:
...
def load_model(args):
    ...
    labels = get_labels(args.labels, args.use_bioul)
    ...
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
                                          ...
                                          cache_dir=None)
    config.use_positions = args.use_positions
    ...
    model = BertLstmCrf(
        ...
        use_cuda=True,
        use_bioul=args.use_bioul,
        labels=args.labels
    )
    checkp = args.model_name_or_path
...
def predict(args, model, tokenizer, mode="test"): 
    if args.local_rank == -1 or args.no_cuda:
        device = torch.device("cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu")
        args.n_gpu = torch.cuda.device_count()
    else:
    ...
    result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, 
                                   mode="test", disable_tqdm=args.disable_tqdm)
    output_test_predictions_file = args.results_file
...
def transformers_ner_crf(args):
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for checkp in checkpoints:
            ...
            # Save predictions
            output_test_predictions_file = os.path.join(checkp, "test_predictions_crf.txt")
