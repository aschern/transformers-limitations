...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.head_learning_rate is None:
        ...
        transformers_params = [(n, p) for n, p in model.named_parameters() if not n.startswith("classifier")]
        head_params = [(n, p) for n, p in model.named_parameters() if n.startswith("classifier")]
        
        optimizer_grouped_parameters = [
            {'params': [p for n, p in transformers_params if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay},
            {'params': [p for n, p in transformers_params if any(nd in n for nd in no_decay)], 'weight_decay': 0.0},
            {'params': [p for n, p in head_params if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay, 'lr': args.head_learning_rate},
            {'params': [p for n, p in head_params if any(nd in n for nd in no_decay)], 'weight_decay': 0.0, 'lr': args.head_learning_rate}
            ]
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.use_length:
                if args.model_type == 'roberta':
                    if inputs['token_type_ids'] is not None:
                        ...
                        inputs['lengths'] = inputs['attention_mask'].sum(dim=1, keepdim=True).float()
            ...
            if args.use_additional_features or args.use_matchings:
                inputs['additional_features'] = batch[4]
...
def evaluate(args, model, tokenizer, prefix="", disable_tqdm=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        preds = None
        out_label_ids = None
        for batch in tqdm(eval_dataloader, desc="Evaluating", disable=disable_tqdm):
            model.eval()
            batch = tuple(t.to(args.device) for t in batch)
        ...
        if args.do_predict and args.results_file:
            with open(args.results_file, 'wb') as f:
                pickle.dump(preds, f)
            with open(args.results_file + '_logits', 'wb') as f:
                pickle.dump(logits, f)
                #pickle.dump([logits[i][preds[i]] for i in range(len(preds))], f)
        if args.do_predict:
...
def get_add_features(sent, processor):
    #labels = np.array(processor.get_labels(), dtype=int)
    #add_features = np.zeros(len(labels))
    #add_features[labels == len(sent.split())] = 1
    add_features = [len(sent.split())]
    return add_features
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, mode=None):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and False:
        ...
        if args.model_type not in ['xlnet']:
            ...
            features = xlnet_convert_examples_to_features(examples,
                                                tokenizer,
                                                label_list=label_list,
                                                max_length=args.max_seq_length,
                                                output_mode=output_mode,
        ...
        if args.use_additional_features:
            # it may be useful to cache them next to labels and load by extra processor method
            # add_features = processor.get_add_features()
            for i in range(len(features)):
                features[i].add_features = get_add_features(examples[i].text_a, processor)
        ...
        if args.use_matchings:
            ...
            ps = PorterStemmer()
            for i in range(len(features)):
                features[i].add_features = get_matchings(examples[i].text_a, train_examples, ps)
    ...
    if args.use_matchings or args.use_additional_features:
        all_add_features = torch.tensor([f.add_features for f in features], dtype=torch.float)
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_add_features)
    else:
...
def transformers_clf(args):
    ...
    config.join_embeddings = args.join_embeddings
    config.use_matchings = args.use_matchings
    config.use_additional_features = args.use_additional_features
    if config.use_matchings or config.use_additional_features:
        config.add_features_size = args.add_features_size
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
                                                do_lower_case=args.do_lower_case,
                                                cache_dir=args.cache_dir if args.cache_dir else None)
    model = model_class.from_pretrained(args.model_name_or_path,
