...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        elif config.use_matchings or config.use_additional_features:
            self.classifier = RobertaClassificationHeadAddFeatures(config)
        else:
            self.classifier = RobertaClassificationHead(config)
    
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, labels=None, lengths=None, embeddings_mask=None, additional_features=None):
        outputs = self.roberta(input_ids,
                               attention_mask=attention_mask,
                               token_type_ids=token_type_ids,
                               position_ids=position_ids,
...
class RobertaClassificationHeadAddFeatures(nn.Module):
    def __init__(self, config):
        super(RobertaClassificationHeadAddFeatures, self).__init__()
        self.dense = nn.Linear(config.hidden_size + config.add_features_size, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(config.hidden_size, config.num_labels)
        self.add_features_size = config.add_features_size

    def forward(self, features, add_features, **kwargs):
        assert add_features.shape[-1] == self.add_features_size, "the expected size of the added features does not match the actual"
        x = features[:, 0, :]
